...
let currentSheetKey = 'hsk1';  // Default sheet
...
function displayRows(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    html += `<tr data-list-idx="${dataListIdx}" data-row="${sheetRow}" data-cn="${escapeHtml(row[0])}" data-vi="${vietnamese}">
                    <td class="hanzi-col"><a class="direct-link" href="chinese-p01.html?word=${chineseEncoded}" target="_blank">${chineseChar}</a></td>
...
async function deleteRow(sheetRow) {
  ...
  try {
    ...
    if (!response || response.status === "error") {
      ...
      // 2. Tự động reload sau khi alert đóng
      setTimeout(() => {
        window.location.reload();
      }, 500);
    }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function createRecognitionCandidate(char) {
    const wrapper = document.createElement('span');
    wrapper.className = 'char-candidate-wrapper';
    ...
    button.type = 'button';
    button.className = 'char-copy-btn';
    button.title = 'Sao chép chữ này';
    button.innerText = '⎘';
    button.addEventListener('click', async (event) => {
      ...
      try {
        ...
        button.innerText = '✓';
        setTimeout(() => { button.innerText = '⎘'; }, 800);
      } catch (error) {
  ...
  async function recognizeStroke() {
    ...
    try {
      ...
      if (data[0] === 'SUCCESS') {
        ...
        recogBox.innerHTML = '';
        // Chỉ lấy 3 chữ nhận dạng giống nhất, kèm nút copy nhỏ
        candidates.slice(0, 3).forEach(char => {
          recogBox.appendChild(createRecognitionCandidate(char));
        });
      }
    } catch (err) { }
  }
